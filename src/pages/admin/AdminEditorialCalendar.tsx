...
export default function AdminEditorialCalendar() {
  ...
  return <div className="p-8 max-w-7xl mx-auto">
    ...
    {error ? <div className="empty-state">Không thể tải lịch biên tập.</div> : <div className="card overflow-hidden"><div className="grid grid-cols-7 border-b" style={{ borderColor: 'var(--border-color)' }}>{['Thứ 2', 'Thứ 3', 'Thứ 4', 'Thứ 5', 'Thứ 6', 'Thứ 7', 'Chủ nhật'].map(day => <div key={day} className="p-3 text-xs font-bold uppercase" style={{ color: 'var(--text-muted)' }}>{day}</div>)}</div><div className="grid grid-cols-7">{isLoading ? Array.from({ length: 35 }, (_, index) => <div key={index} className="min-h-32 p-2 border-b border-r" style={{ borderColor: 'var(--border-color)' }}><div className="skeleton h-4 w-8" /></div>) : days.map(day => { const dayPosts = postsForDay(day); const outside = day.getMonth() !== month.getMonth(); return <div key={day.toISOString()} className={`min-h-32 p-2 border-b border-r ${outside ? 'opacity-40' : ''}`} style={{ borderColor: 'var(--border-color)' }}><div className="flex items-center justify-between mb-2"><span className={`text-sm font-bold ${isSameDay(day, new Date()) ? 'text-blue-400' : ''}`}>{format(day, 'd')}</span>{dayPosts.length > 0 && <span className="text-[.65rem]" style={{ color: 'var(--text-muted)' }}>{dayPosts.length} bài</span>}</div><div className="space-y-1">{dayPosts.slice(0, 3).map(post => <Link key={post.id} to={`/admin/posts/${post.id}/edit`} className={`block rounded-md px-2 py-1 text-[.68rem] truncate ${post.status === 'scheduled' ? 'bg-blue-500/15 text-blue-300' : post.status === 'published' ? 'bg-green-500/15 text-green-300' : 'bg-orange-500/15 text-orange-300'}`}>{post.title}</Link>)}{dayPosts.length > 3 && <span className="text-[.65rem]" style={{ color: 'var(--text-muted)' }}>+ {dayPosts.length - 3} bài khác</span>}</div></div> })}</div></div>}
